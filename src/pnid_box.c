...
/* pnid_box_is_subset(): true if and only if a⊆mbr.

   ie. true when the area of rectangle a is contained within that of
   a the bounding rectangle br. */
int
pnid_box_is_subset(const PnidBox *a, const PnidBox *br)
{
  return
    pnid_box_get_left(a)   >=  pnid_box_get_left(br)  &&
    pnid_box_get_top(a)    >=  pnid_box_get_top(br)   && 
    pnid_box_get_right(a)  <=  pnid_box_get_right(br) &&
    pnid_box_get_bottom(a) <=  pnid_box_get_bottom(br);
}

/* pnid_box_is_separate(): true when a∩b=∅ i.e. there is no
   intersection in the areas of rectangles a and b */
int
